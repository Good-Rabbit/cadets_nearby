      ...
      children: [
        ...
        StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
            stream: HomeSetterPage.store
                .collection('offers')
                .where('long',
                    isLessThan: context.read<MainUser>().user!.long +
                        0.0543,
                    isGreaterThan: context.read<MainUser>().user!.long -
                        0.0543)
                .where('latsector', whereIn: whereInLat)
                .snapshots(),
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                List<QueryDocumentSnapshot<Map<String, dynamic>>> offers = snapshot.data!.docs;
